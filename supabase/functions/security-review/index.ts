...
const STATIC_FINDINGS: Finding[] = [
  ...
  },
  {
    ...
    issue: "verify_jwt=false in config.toml for daily-scan, scheduled-scan, and others. User-facing functions (generate-cv, cover-letter, interview-prep, company-research, update-job-statuses) now perform explicit JWT validation inside the handler.",
    risk: "Functions with verify_jwt=false and no internal auth check could be invoked without a valid token. Cron-only functions (scheduled-scan) are intentionally unauthenticated.",
    recommendation: "Audit remaining verify_jwt=false functions. For each: either add internal JWT check or confirm it's cron-only with a shared secret guard.",
    auto_fix_allowed: false,
    verified: false,
  },
  {
    ...
    issue: "✅ RESOLVED — Per-user rate limiting added to all generation functions (10 calls/hr for CV/cover-letter/interview-prep, 20 calls/hr for company-research).",
    risk: "Rate limiting is now enforced at the DB level via the rate_limit_checks table.",
    recommendation: "Monitor the rate_limit_checks table for abuse patterns. Consider lowering limits if Claude API costs spike.",
    auto_fix_allowed: false,
    verified: false,
  },
  {
  ...
  },
  {
    ...
    area: "generate-cv, generate-cover-letter, interview-prep, company-research",
    issue: "✅ RESOLVED — UUID format validation added to all generation functions. jobId is validated against /^[0-9a-f]{8}-...-[0-9a-f]{12}$/i before any DB query. All job fetches include .eq('user_id', userId) ownership check.",
    risk: "Resolved. Both validation layers (format check + ownership check) are now in place.",
    recommendation: "No further action needed. Keep UUID_RE validation in place for any new functions that accept jobId.",
    auto_fix_allowed: false,
    verified: false,
  },
  {
